...
var SITE_SHOP_DETAIL = function () {
    ...
    */
     ...
     */
    var _addOrder = function (type, backurl, is_oms, callback){
    ...
    var handleOmsDigitalDownload = function (e, href) {
        ...
        if (now - lastOmsDownloadClick < 500) {
            // alert을 노출하는게 맞을까?
            // alert('다운로드 요청 중입니다. 잠시만 기다려주세요.');
            return;
        }
        ...
        if (!options.is_price_view_permission && window.IS_GUEST) {
            ...
            if (cm_data.use_login_popup === 'Y') {
                window.SITE_MEMBER.openLogin(
                    ...
                    'payment',
                    function () {
                        moveOrder('normal', encodeURIComponent(base64Encode(window.location.href))) 
                    },
                    'N',
                    'payment',
                );
